...
void CXTabCtrl::AddTab(CString szTitle, int iImage)
{
	TCITEM item;
	item.mask = TCIF_TEXT|TCIF_PARAM|TCIF_IMAGE;
	//item.lParam = (LPARAM) pWnd;
	item.pszText = szTitle.GetBuffer(0);
	item.iImage = iImage;
...
{	
	...
	if (!IsTabEnabled(nTabIndex))
	{
		...
		//pDC->TextOut(rect.TopLeft().x, rect.TopLeft().y, label);	
	}
	else
	{
		//** selected item -----
...
void CXTabCtrl::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	// TODO: Add your message handler code here and/or call default
	if (m_iIndexMouseOver != -1)
	{
