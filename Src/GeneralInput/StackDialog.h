...
class TStackedPage : public CXTResizeDialog
{
public:
	TStackedPage(UINT nId, CWnd* pParent = NULL);
	virtual ~TStackedPage();
	
	BOOL m_bModified;
	
	void SetModified(BOOL bVal=TRUE) { m_bModified = bVal; }  
	virtual void OnPreInitDialog(){};
	virtual void DrawInputDomyunView(BOOL bZoomAll=TRUE){};
	virtual void SetDataDefault(){};
	virtual void ApplyIdentityAllGirder(){};
	virtual void ApplyIdentityPart(){};
	virtual void ApplyIdentityAllJijum(){};
	virtual void ApplyCompleteInputAllSolePlate(){};
	virtual void SetDataJISRecommend(){};
	virtual void SetDataKSRecommend(){};
	virtual void SetDataSave(){};
	virtual void SetDataInit()=0;
...
class AFX_EXT_CLASS TStackedDialog : public CGeneralBaseDlg
{
	DECLARE_DYNAMIC(TStackedDialog);
...
public:
	TStackedDialog(UINT nPlaceholder, UINT nId, CWnd* pParent = NULL);
