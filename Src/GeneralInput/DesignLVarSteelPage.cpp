// DesignLVarSteelPage.cpp : implementation file
//
...
CDesignLVarSteelPage::CDesignLVarSteelPage(CDataManage *pMng, CWnd* pParent /*=NULL*/)
	: TStackedPage(CDesignLVarSteelPage::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDesignLVarSteelPage)
	m_nIndex = 0;
	//}}AFX_DATA_INIT
	m_pDataManage = pMng;
...
void CDesignLVarSteelPage::DoDataExchange(CDataExchange* pDX)
{
	TStackedPage::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDesignLVarSteelPage)
	DDX_GridControl(pDX, IDC_GRID_LVARSTEEL, m_Grid);
	DDX_GridControl(pDX, IDC_GRID_LVARSTEEL_PROPERTY, m_Grid_Property);
	//}}AFX_DATA_MAP
	DDX_DomControl(pDX, IDC_LVARSTEEL_DOMYUN, m_Domyun);
}


BEGIN_MESSAGE_MAP(CDesignLVarSteelPage, TStackedPage)
	//{{AFX_MSG_MAP(CDesignLVarSteelPage)
		// NOTE: the ClassWizard will add message map macros here
	//}}AFX_MSG_MAP
	ON_NOTIFY(GVN_CELL_CHANGED_ROW		, IDC_GRID_LVARSTEEL, OnCellChangedRow)
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CDesignLVarSteelPage message handlers
void CDesignLVarSteelPage::OnPreInitDialog()
{
	SetResize(IDC_GRID_LVARSTEEL,				SZ_TOP_LEFT, SZ_BOTTOM_RIGHT);
	SetResize(IDC_GRID_LVARSTEEL_PROPERTY,	SZ_TOP_RIGHT, SZ_BOTTOM_RIGHT);
	SetResize(IDC_LVARSTEEL_DOMYUN,			SZ_TOP_RIGHT, SZ_TOP_RIGHT);
}

void CDesignLVarSteelPage::SetDataDefault()
{	
}

void CDesignLVarSteelPage::SetDataInit()
{
	SetGridTitle();
	SetLVarSteelProperty();
	DrawLVarSteel();

	CSteelMaterial *pSteelMat = m_pDataManage->GetSteelMaterial();

	for(long n=0; n<pSteelMat->m_MatDataArray[CSteelMaterialData::LVALSTEEL].GetSize(); n++)
	{
		CSteelMaterialData *pMatData = pSteelMat->m_MatDataArray[CSteelMaterialData::LVALSTEEL].GetAt(n);		
		long nCol=0;
		...
		m_Grid.SetTextMatrix(n+2,nCol++,"%g",pMatData->m_dTanAlpha);
		*/
	}
}

void CDesignLVarSteelPage::SetGridTitle()
{
	...
	m_Grid.SetRowCount(m_pDataManage->GetSteelMaterial()->m_MatDataArray[CSteelMaterialData::LVALSTEEL].GetSize()+2);
...
/*
	...
	m_Grid.SetTextMatrix(0, 17, "Tan α");
	m_Grid.SetMergeRow(0,1,17);

	m_Grid.SetTextMatrix(0, 18, "단면계수(cm3)");
	m_Grid.SetMergeCol(0,18,19);
	m_Grid.SetTextMatrix(1, 18, "Zx");
	m_Grid.SetTextMatrix(1, 19, "Zy");
*/
...
//////////////////////////////////////////////////////////////////////
	...
	m_Grid_Property.SetTextMatrix(8, 0, " · Tan α", 0, DT_LEFT);
	m_Grid_Property.SetMergeCol(8, 0, 1);
	m_Grid_Property.SetItemBkColour(8, 0, Bakckcolor);

	m_Grid_Property.SetTextMatrix(10, 0, " · 단면계수 ( cm³)", 0, DT_LEFT);
	m_Grid_Property.SetMergeCol(10, 0, 1);
	m_Grid_Property.SetItemBkColour(10, 0, Bakckcolor);

	m_Grid_Property.SetRedraw(TRUE,TRUE);
}

BOOL CDesignLVarSteelPage::OnInitDialog() 
{
...
void CDesignLVarSteelPage::SetLVarSteelProperty()
{
	CSteelMaterial		*pSteelMat	= m_pDataManage->GetSteelMaterial();
	CSteelMaterialData	*pMatData	= pSteelMat->m_MatDataArray[CSteelMaterialData::LVALSTEEL].GetAt(m_nIndex);
	CString				sText;
	...
	// Tan α
	sText.Format("   α      = %.3f", pMatData->m_dTanAlpha);
	m_Grid_Property.SetTextMatrix(9, 0, sText, 0, DT_LEFT);

	// 단면계수
	sText.Format("   Zx      = %.3f", pMatData->m_dModulusOfSection.x);
	m_Grid_Property.SetTextMatrix(11, 0, sText, 0, DT_LEFT);
	sText.Format("   Zy      = %.3f", pMatData->m_dModulusOfSection.y);
	m_Grid_Property.SetTextMatrix(11, 1, sText, 0, DT_LEFT);

	m_Grid_Property.SetRedraw(TRUE,TRUE);
}

void CDesignLVarSteelPage::DrawLVarSteel()
{
	CSteelMaterial		*pSteelMat	= m_pDataManage->GetSteelMaterial();
	CSteelMaterialData	*pMatData	= pSteelMat->m_MatDataArray[CSteelMaterialData::LVALSTEEL].GetAt(m_nIndex);
...
//	Dom.LineTo(vX*dB*2/3/2-vY*dH*1/3, vX*dB*2/3/2+vY*(dH+dH*1/3));
	...
	double dScale = 3*(dDiagonalL/125);
...
void CDesignLVarSteelPage::OnCellChangedRow(NMHDR* nmgv, LRESULT*)
{
