// PlateSymbolDom.cpp: implementation of the CAPlateSymbolDom class.
//
//////////////////////////////////////////////////////////////////////
...
CAPlateSymbolDom::CAPlateSymbolDom(CDomyun *pDom, CDataManage *pDataMng) :
				  CDomyun(pDom), m_pDataMng(pDataMng)
{

}

CAPlateSymbolDom::~CAPlateSymbolDom()
{
...
// Function
//////////////////////////////////////////////////////////////////////
void CAPlateSymbolDom::DrawWeldMark(CDPoint xy,long nMarkBuje1, long nMarkBuje2, CPlateBasicIndex *pBx, double dWeldSize, double dHeight1, double dHeight2, BOOL bLeft, BOOL bUpper, double dAngle, BOOL bFrontFace, CString szAddStr, BOOL bGrind, BOOL bSttSplice, BOOL bFlag, BOOL bUpperSide, long nIdx)
{
	...
	if (nIdx == -99)//현장용접이 아니면...
	{
		nIdx = pBridge->GetIndexBaseWeld(nMarkBuje1,nMarkBuje2,pBx);
		if(nIdx==-1)
			nIdx = pBridge->GetIndexBaseWeld(nMarkBuje2,nMarkBuje1,pBx);
	}	
	CSteelWeldData *pWeld = m_pDataMng->GetSteelWeld()->GetBaseWeldByIndex(nIdx);
	...
	switch(pWeld->m_nWeldType)
	{
		case 0:
			if(pWeld->m_nLayType==1)
				...
				nType = FILLET_U;
			break;
		// 맞대기용접일때 더 얇은 부재두께(-2)를 용접두께로 낸다.
		// 단, 하나가 주형일 경우 부부재의 두께로 내고, 둘다 주형일경우 더 두꺼운부재를 용접두께로 냄.
		case 1:
			T1 = pGir->GetThickJewon(nMarkBuje1,pBx);
			T2 = pGir->GetThickJewon(nMarkBuje2,pBx);
			
			if((nMarkBuje1 == G_F_U || nMarkBuje1 == G_F_L || nMarkBuje1 == G_W) &&
			   (nMarkBuje2 == G_F_U || nMarkBuje2 == G_F_L || nMarkBuje2 == G_W))		dWeldSize = max(T1,T2);
			else if(nMarkBuje1 == G_F_U || nMarkBuje1 == G_F_L || nMarkBuje1 == G_W)	dWeldSize = T2-2;
			else if(nMarkBuje2 == G_F_U || nMarkBuje2 == G_F_L || nMarkBuje2 == G_W)	dWeldSize = T1-2;
			else dWeldSize = min(T1,T2)-2;
			break;
		case 4:
			nType = ROUNDWELD;
			break;
	...
	{
		case 0:
			if(pWeld->m_nLayType==1)	nType = ITYPE;
			else if(bFrontFace)			nType = ITYPE_L;
			else						nType = ITYPE_U;
			break;
		case 1:
			if(pWeld->m_nLayType==1)	nType = KWELD;
			else if(bFrontFace)			nType = KWELD_L;
			else						nType = KWELD_U;
			break;		
		case 2:
			nType = KWELD;
			...
			break;
		case 3:			
			if(pWeld->m_nLayType == 0)		nType = XWELD_L;
			else							nType = XWELD_U;
			...
			break;		
		case 4:
			if(nMarkBuje1==G_W && nMarkBuje2==G_W)	nType = XWELD_W;
			else if(bFrontFace)						nType = XWELD_UF;
			else									nType = XWELD_LF;
			break;
		case 5://현장용접...
			if((nMarkBuje1==G_F_U || nMarkBuje1==G_F_L || nMarkBuje1==G_W) && (nMarkBuje2==G_F_U || nMarkBuje2==G_F_L || nMarkBuje2==G_W))
			{
				T1 = pBx->GetFactChain(nMarkBuje1,FALSE)->m_dFactChainThick;
				T2 = pBx->GetFactChain(nMarkBuje2,TRUE)->m_dFactChainThick;	
				bWeldByValue = FALSE;
	...
	double dThickNoImprove = pGlopt->GetDesignThickWeldNoCut();
	...
	if(bWeldByValue)
		DomSub.WeldByValue(xy.x, xy.y, dWeldSize, nType, bLeft, bUpper, dHeight1, dHeight2, bGrind, bOnDule, szAddStr, bFlag, bUpperSide);
	else
		DomSub.Weld(xy.x, xy.y, T1, T2, nType, bLeft, bUpper, dHeight1, dHeight2, bGrind, bOnDule, szAddStr, bFlag, bUpperSide);			
...
/*        strTitle1                                         strTitle2
           ...
           |---|              -                              |---| 
			 ...
			 ---------------x         X xy      x---------------
*/
void CAPlateSymbolDom::DrawSymbolSectionGisiArrow(CDPoint xy, double dDis, CString strTitle1, CString strTitle2, double dRotAng)
{
	CARoadOptionStd *pOptStd = m_pDataMng->GetOptionStd();

	CDomyun Dom(this);
	pOptStd->SetEnvType(&Dom,HCAD_SYMB);
	double dXLen = Dom.Always(10), dYLen = Dom.Always(10), dTriXLen = Dom.Always(3), dTriYLen = Dom.Always(5), dMar = Dom.Always(0.5);
	Dom.SetTextHeight(5);
	...
	Dom.TextOut(xy2.x+dXLen, xy2.y + dYLen + dTriYLen + dMar, strTitle2);
...
void CAPlateSymbolDom::DrawSymbolTitle(double xpos, double ypos, CString Title, CString UpStr, CString DnStr, CString DnTitle)
{
...
void CAPlateSymbolDom::DrawSymbolSPMark(double xpos, double ypos, double Height, CString Str, double AngDegRotate, BOOL bBlock, BOOL bUpper, BOOL bSmall)
{
	CDomyun Dom(this);	

	Dom.SetTextAlignHorz(TA_CENTER);

	Dom.LineTo(0,0,0,Height);	
	double radius = bSmall ? Dom.GetTextHeight()*0.8
		                   : Dom.GetTextHeight()*1.3;
	if(bUpper) {
		Dom.TextCircle(0,Height,Str,2,TRUE,bSmall);
		Dom.Rotate(AngDegRotate,0,0);
	}
	else {
		Dom.TextCircle(0,Height-radius*2,Str,2,TRUE,bSmall);
		Dom.Rotate(AngDegRotate,0,0);
	}
...
// AngDegRotate : 선형기준으로 회전
void CAPlateSymbolDom::DrawSymbolJijumMark(double xpos, double ypos, CString Str, double AngDegRotate, BOOL bBlock)
{
...
void CAPlateSymbolDom::SymbolTextCircle(CDPoint xy, CString str, CString sBlockName, CDPoint vAngRot, BOOL bSameSize)
{
	CDomyun *pDom = new CDomyun(this);
	pDom->SetTextAlignHorz(TA_CENTER);
...
void CAPlateSymbolDom::SymbolGiSiArrowExtend(CDPoint xy, double Dis1, double Dis2, long nDirArrow, 
...
// AngDegRotate : 선형기준으로 회전
void CAPlateSymbolDom::DrawSymbolGirderMark(CDomyun *pCheckDom, double xpos, double ypos, CString Str, double AngDegRotate, BOOL bBlock, long nInit /* = 0 */)
{	
	CDomyun *pDom = new CDomyun(this);
	pDom->SetTextAlignHorz(TA_CENTER);
...
void CAPlateSymbolDom::DrawSymbolShopMark(double xpos, double ypos, CString Str, double AngDegRotate, double dWidth, BOOL bBlock, BOOL bEllipse)
{
	CDomyun Dom(this);
	Dom.SetTextAlignHorz(TA_CENTER);
...
void CAPlateSymbolDom::DrawSymbolDetailMark(double xpos, double ypos, double Height, CString UpStr, 
...
void CAPlateSymbolDom::DrawSymbolNote1(double xpos, double ypos)
{	
	// 도면 이동, 스케일조정
	CARoadOptionStd *pOptStd = m_pDataMng->GetOptionStd();

	CGridDomyun p;				
	pOptStd->SetEnvType(&p,HCAD_TBL1);
...
void CAPlateSymbolDom::DrawSymbolNote2(double xpos, double ypos)
{	
	CARoadOptionStd *pOptStd = m_pDataMng->GetOptionStd();
...
void CAPlateSymbolDom::DrawSymbolVentPlan(CDPoint xy, CDPoint rvAng, BOOL bText, BOOL bUpText)
{
	CARoadOptionStd *pOptStd = m_pDataMng->GetOptionStd();
...
void CAPlateSymbolDom::DrawSymbolCLMark(double xpos, double ypos, BOOL VerDirection, BOOL bBlock)
{	
	CARoadOptionStd *pOptStd = m_pDataMng->GetOptionStd();

	CDomyun *pDom = new CDomyun(this);
	pOptStd->SetEnvType(pDom,HCAD_SYMB);
...
void CAPlateSymbolDom::DrawSymbolDanMyunMark(double xpos, double ypos, CString cs, long Dir, BOOL bBlock)
{	
	CARoadOptionStd *pOptStd = m_pDataMng->GetOptionStd();
...
void CAPlateSymbolDom::DrawSymbolThickMark(double xpos, double ypos, CString cs, BOOL bBlock)
{
	CARoadOptionStd *pOptStd = m_pDataMng->GetOptionStd();
...
void CAPlateSymbolDom::DrawSymbolCuttingS(double xpos, double ypos, double Angle)
{
	CDomyun *pDom = new CDomyun(this);
...
void CAPlateSymbolDom::DrawSymbolCircleMark(double xpos, double ypos, double Height, CString UpStr, 
									   CString DnStr, CString csText, BOOL bUpper, BOOL bLeft, double Ang)
{
	CDomyun ArrowDom(this), TextDom(this);
...
//        -   -
//          -
void CAPlateSymbolDom::DrawSymbolDiamondMark(CDPoint xy, CDPoint rAng, CString Str, BOOL bBlock)
{
	long nID = 0;
	CDomyun *pDom = new CDomyun(this);
...
void CAPlateSymbolDom::DrawSymbolHexagonStr(CDPoint xy, CDPoint rAng, CString str, BOOL bBlock)
{
	long nID = 0;
	CDomyun *pDom = new CDomyun(this);
...
void CAPlateSymbolDom::DrawSymbolGiSiLine(double X , double Y, double Dis, BOOL bLeft, BOOL bUpper, double Height1, double Height2, CString cs)
{
...
void CAPlateSymbolDom::DrawSymbolBoltBox(double xpos, double ypos, CString str1, CString str2)
{
...
void CAPlateSymbolDom::DrawSymbolTitleExt(double xpos, double ypos, long nSu, CString Title, CString sAssem, CString UpStr, CString DnStr)
{
	CDomyun Dom(this);	
	CString sTitle;
...
void CAPlateSymbolDom::DrawSymbolShoeMarkPlan(long nRow, long nCol, BOOL bBlock, BOOL bMoveAndRotate, BOOL bScaleApply)
{
	/*
	CPlateBridgeApp	*pBridge = m_pDataMng->GetBridge();
	...
	long nG = pBridge->GetNumberGirderByShoe(nRow,nCol);
	CDiaphragm *pDia = pBridge->GetDiaphragmByJijumNo(nG,nCol);

	CShoe *pShoe = pBridge->IsFirstShoe(nRow,nCol) ? &pDia->m_ShoeFir : &pDia->m_ShoeSec;
	...
	CDPoint xy   = pBridge->GetXyShoe(nRow,nCol);
	CDPoint rAng = pBridge->GetGirder(nG)->GetLine()->GetAngleAzimuth(pBridge->GetStationShoeOnGirCenter(nRow,nCol));

	if(bMoveAndRotate)
	{
		pDom->Rotate(CDPoint(0,0),rAng);
		double Angle = pBridge->GetAngleShoeOnGirCenter(nRow,nCol).GetAngleRadian() - ConstPi/2;
		pDom->Rotate(Angle,0,0);
	}
	else
		xy = CDPoint(0,0);
...
void CAPlateSymbolDom::DrawSymbolShoeMarkPlanGen(double x, double y, long nType, double DegAng)
{
...
// Length=-1 : 자동산정
void CAPlateSymbolDom::DrawSymbolStationFlag(CDPoint xy, CString strUpper, CString strLower, 
...
void CAPlateSymbolDom::DrawSymbolCuttingMark(CDPoint Point, double dRadius, double Angle)
{
	CDomyun Dom(this);		
...
void CAPlateSymbolDom::DrawTitleOnTable(double xp, double yp, CString sTitle, long nLayerType, BOOL bTopPos, long nQtyLine)
{ 		
	CARoadOptionStd *pOptStd = m_pDataMng->GetOptionStd();
	...
	pOptStd->SetEnvType(pDom,nLayerType);
...
void CAPlateSymbolDom::SymbolTextCircle(CDPoint xy, CString str, double Radius, double TextHeight)
{
	CDomyun* pDom = new CDomyun(this);		
	pDom->SetTextAlignHorz(TA_CENTER);
...
void CAPlateSymbolDom::SymbolTextCircleWithLine(CDPoint xy, CString str, CDPoint LineStt, double Radius, double TextHeight)
{
	CDomyun* pDom = new CDomyun(this);		
	pDom->SetTextAlignHorz(TA_CENTER);
...
void CAPlateSymbolDom::DrawSymbolSectionMark(double BX, double BY, CString csText, BOOL bLeftText/*TRUE*/, double AngleDeg, double TextAngleDeg)
{
	CDomyun *pDom = new CDomyun(this);
...
void CAPlateSymbolDom::DrawSymbolDirectionArrow(CDPoint cp, CString cs, BOOL bFront)
{
	CDomyun *pDom = new CDomyun(this);
	pDom->SetTextAlignHorz(TA_CENTER);
...
/*
   ...
   |----- sText ----|
                   |
*/
void CAPlateSymbolDom::DrawSymbolPropName(double xpos, double ypos, CString sText, double Angle)
{
	CARoadOptionStd *pOptStd = m_pDataMng->GetOptionStd();
...
void CAPlateSymbolDom::DrawSymbolLinePointMark(CDPoint bp, CDPoint ep, CString szSta, CString szTypeText, CString szLeftType, CString szRightType)
{
	CDomyun Dom(this);
	Dom.SetTextAlignHorz(TA_CENTER);
...
void CAPlateSymbolDom::DrawSymbolTitleExt(double xpos, double ypos, CString Title, CStringArray &strArr)
{
...
void CAPlateSymbolDom::SymbolTextCircle(CDPoint xy, CString str, BOOL bSameSize)
{
	CDomyun* pDom = new CDomyun(this);		
